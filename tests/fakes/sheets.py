...
class FailurePlan:
    ...
    Keys use the operation name and one-based call number, for example
    ``("append_row", 1)``. Tests can therefore fail every mutation boundary
    without using credentials or a network connection.
    """
