...
# Helper for clean category name.
...
# Helper for category flow keyboard.
...
# Helper for format category rows for help.
...
# Helper for format alias preview.
...
# Helper for build category list text.
def _build_category_list_text(records: list[dict]) -> str:
    ...
    # Validate missing any(grouped.values()) before continuing.
...
# Helper for build symbol prompt.
...
# Helper for format aliases for message.
...
# Helper for category confirm target.
...
# Helper for build category preview text.
...
async def _accept_category_name(
    ...
    *,
    ...
    """
    # Normalize clean name before matching.
...
async def handle_pending_category_flow(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str) -> bool:
    ...
    if stage == "name":
        # Await  accept category name before continuing.
    ...
    if stage == "symbol":
        ...
        if mode == "add":
            # Await  prepare new category preview before continuing.
    ...
    if stage == "aliases":
        # Await  prepare edited category preview before continuing.
...
async def _prepare_edited_category_preview(update: Update, context: ContextTypes.DEFAULT_TYPE, state: dict, raw_aliases: str) -> None:
    ...
    # Normalize clean low before matching.
