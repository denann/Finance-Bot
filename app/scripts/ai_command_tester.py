...
class _Dummy:
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Helper for init in the developer utility script."""
        self.args = args
        self.kwargs = kwargs
    ...
    def __call__(self, *args: Any, **kwargs: Any) -> "_Dummy":
        """Helper for call in the developer utility script."""
        return _Dummy(*args, **kwargs)

    def __iter__(self):
        """Helper for iter in the developer utility script."""
        return iter([])

    def __bool__(self) -> bool:
        """Helper for bool in the developer utility script."""
        return False
...
def _module_exists(module_name: str) -> bool:
    """Helper for module exists in the developer utility script."""
    try:
...
def _install_optional_import_stubs() -> None:
    """Helper for install optional import stubs in the developer utility script."""
    ...
    if not _module_exists("google.oauth2.service_account"):
        ...
        class _Credentials:
            ...
            def from_service_account_file(cls, *args: Any, **kwargs: Any) -> "_Credentials":
                """Helper for from service account file in the developer utility script."""
                return cls()
    ...
    if not _module_exists("langchain_google_genai"):
        ...
        class ChatGoogleGenerativeAI:
            ...
            def invoke(self, *args: Any, **kwargs: Any) -> Any:
                """Helper for invoke in the developer utility script."""
...
def classify_known_route(text: str) -> dict[str, Any] | None:
    """Helper for classify known route in the developer utility script."""
    raw = (text or "").strip()
    low = raw.lower()
    if not raw:
        return None
...
class CommandTester:
    ...
    def _import_handlers(self):
        """Helper for import handlers in the developer utility script."""
        try:
    ...
    def _jsonable_item(self, item: dict[str, Any]) -> dict[str, Any]:
        """Helper for jsonable item in the developer utility script."""
    ...
    def _jsonable_items(self, items: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Helper for jsonable items in the developer utility script."""
...
def compare_value(actual: Any, expected: Any) -> bool:
    """Helper for compare value in the developer utility script."""
...
def evaluate_expectations(run: CommandRun, expect: dict[str, Any] | None) -> list[AssertionResult]:
    """Helper for evaluate expectations in the developer utility script."""
...
def _has_split_keyword(text: str) -> bool:
    """Check whether data has split keyword."""
...
def _split_has_friend_name(text: str) -> bool:
    """Helper for split has friend name in the developer utility script."""
...
def evaluate_heuristics(run: CommandRun) -> list[AssertionResult]:
    """Helper for evaluate heuristics in the developer utility script."""
    results: list[AssertionResult] = []
...
def case_status(assertions: list[AssertionResult]) -> str:
    """Helper for case status in the developer utility script."""
...
def deterministic_diagnosis(run: CommandRun, assertions: list[AssertionResult]) -> str:
    """Helper for deterministic diagnosis in the developer utility script."""
...
def ai_diagnosis(run: CommandRun, assertions: list[AssertionResult]) -> str:
    """Helper for ai diagnosis in the developer utility script."""
...
def command_run_to_dict(run: CommandRun) -> dict[str, Any]:
    """Helper for command run to dict in the developer utility script."""
    return {
        "name": run.name,
...
def load_text_cases(path: Path, *, decision: str | None = None) -> list[dict[str, Any]]:
    ...
    def flush() -> None:
        """Helper for flush in the developer utility script."""
...
def default_sample_cases() -> list[dict[str, Any]]:
    """Helper for default sample cases in the developer utility script."""
    return [
        {
...
def write_sample(path: Path) -> None:
    """Helper for write sample in the developer utility script."""
...
@dataclass
class CaseResult:
    """Summary model for one test case execution."""
    run: CommandRun
...
def print_case_report(result: CaseResult, *, show_json: bool, use_ai: bool) -> None:
    """Helper for print case report in the developer utility script."""
    run = result.run
...
def make_markdown_report(results: list[CaseResult]) -> str:
    """Helper for make markdown report in the developer utility script."""
    total = len(results)
...
def parse_args() -> argparse.Namespace:
    """Parse input into structured data for args."""
...
def main() -> int:
    """Helper for main in the developer utility script."""
    args = parse_args()
