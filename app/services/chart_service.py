...
# Helper for get chart effective expense amount.
...
# Helper for daily net expense series.
def daily_net_expense_series(report: dict) -> list[float]:
    ...
    for txn in (report or {}).get("transactions", []) or []:
        ...
        # Handle day is None or day < 1 or day > days.
...
# Helper for transaction net expense series.
...
# Helper for build transaction timeseries png bytes.
...
# Helper for write transaction timeseries png.
...
# Helper for category net expense items.
def category_net_expense_items(report: dict, limit: int = 8) -> list[tuple[str, float, float]]:
    ...
    rows = [
        ...
        # Iterate through each category, net.
...
# Helper for load matplotlib pyplot.
...
# Helper for configure matplotlib style.
...
# Helper for finalize figure to png bytes.
...
# Helper for build empty chart png bytes.
...
# Helper for build monthly timeseries png bytes.
...
# Helper for build monthly bar png bytes.
def build_monthly_bar_png_bytes(report: dict) -> bytes:
    ...
    # Iterate through each position, row.
...
# Helper for build monthly pie png bytes.
