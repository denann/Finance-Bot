...
# Helper for install optional import stubs.
...
def classify_known_route(text: str) -> dict[str, Any] | None:
    ...
    # Iterate through each pattern, intent.
...
class CommandTester:
    ...
    def run_command(self, input_text: str, *, name: str = "manual", decision: str | None = None) -> CommandRun:
        ...
        if routed is not None and "\n" not in raw_text and ";" not in raw_text:
            return CommandRun(
                name=name,
                # Prepare input text from the incoming input.
        ...
        if mode == "batch":
            ...
            # Extract account needed for validation.
...
def get_path(data: Any, path: str) -> Any:
    ...
    """
    current = data
    # Validate missing path before continuing.
    if not path:
        return current
    for token in path.split("."):
        if isinstance(current, list):
            ...
            # Handle an expected failure from the guarded operation above.
                ...
                return None
        # Fall back when isinstance(current, dict).
...
# Helper for evaluate expectations.
def evaluate_expectations(run: CommandRun, expect: dict[str, Any] | None) -> list[AssertionResult]:
    ...
    # Validate missing expect before continuing.
    if not expect:
        return results
...
# Helper for split has friend name.
...
# Helper for evaluate heuristics.
def evaluate_heuristics(run: CommandRun) -> list[AssertionResult]:
    ...
    # Validate missing run.items before continuing.
    if not run.items:
    ...
    for idx, item in enumerate(run.items, 1):
        ...
        if incoming_match and not re.search(r"\bdari\s+[^\n]+?\s+ke\s+", raw, flags=re.IGNORECASE):
            ...
            # Handle first token and first token not in own accounts.
    ...
    for idx, item in enumerate(split_items, 1):
        ...
        # Fall back when not has friend and not split.
...
# Helper for deterministic diagnosis.
def deterministic_diagnosis(run: CommandRun, assertions: list[AssertionResult]) -> str:
    ...
    # Validate missing failed and not warnings before continuing.
    ...
    # Validate missing tips before continuing.
    if not tips:
...
# Helper for command run to dict.
...
def resolve_input_path(path_text: str) -> Path:
    ...
    candidates = [raw]
    # Validate missing raw.is absolute() before continuing.
...
def load_text_cases(path: Path, *, decision: str | None = None) -> list[dict[str, Any]]:
    ...
    # Prepare raw lines from the incoming input.
    ...
    # Validate missing has blocks before continuing.
    if not has_blocks:
    ...
    def flush() -> None:
        ...
        for line in current_lines:
            stripped = line.strip()
            # Validate missing stripped before continuing.
            if not stripped:
                # Skip the rest of this loop iteration after handling this case.
                continue
        ...
        # Build current lines for the response flow.
        current_lines = []
        current_name = None
    ...
    for line in raw_lines:
        ...
        # Append the current value to current lines.
...
# Helper for default sample cases.
...
# Helper for make markdown report.
def make_markdown_report(results: list[CaseResult]) -> str:
    ...
    if fail_count or warn_count:
        ...
        lines.append("")
        # Iterate through each idx, result.
...
def run_cases(cases: list[dict[str, Any]], *, show_json: bool, use_ai: bool, markdown_path: Path | None = None) -> int:
    ...
    # Handle fail count == 0 and warn count == 0.
    if fail_count == 0 and warn_count == 0:
        print("Semua test case lolos.")
    # Fall back when fail count == 0.
    elif fail_count == 0:
